...
The static file server will run on http://localhost:8080/ . The app server will run on http://localhost:8081/ .

If you go to http://localhost:8080/ , hopefully the app should work. The client ordinarily wants the app server to be running on the same host as the static file server, at the path "/api/". However, as a special case, it will automatically notice when it's running on localhost, and switch to assuming the app server is at http://localhost:8081/ instead.
